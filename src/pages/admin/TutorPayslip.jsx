  ...
  useEffect(() => {
    const convertImageToBase64 = async (url, setBase64) => {
      ...
      try {
        ...
        setBase64(absoluteUrl);
      }
    };
    convertImageToBase64('/logo.png', setLogoDataUrl);
    if (tutor?.qr_code_url) convertImageToBase64(tutor.qr_code_url, setQrDataUrl);
  }, [tutor?.qr_code_url]);

  const toggleGroup = (groupId) => setExpandedGroups(prev => prev.includes(groupId) ? prev.filter(i => i !== groupId) : [...prev, groupId]);
  const toggleRow = (logId) => setExpandedLogs(prev => prev.includes(logId) ? prev.filter(i => i !== logId) : [...prev, logId]);
  ...
  return (
    <div className="bg-white w-full h-auto flex flex-col p-3 sm:p-4 mx-auto text-gray-900 font-sans text-[10px] sm:text-[11px] select-none leading-tight">
      ...
      <div className="h-auto my-1 flex flex-col border border-gray-200 rounded">
                  ...
                  {expandedGroups.includes(group.id) && group.sessions.map((session, index) => (
                    <React.Fragment key={session.id}>
                      ...
                      {expandedLogs.includes(session.id) && (
                        <tr className="bg-white border-b border-gray-100 border-l-2 border-[#1b4379]"><td className="border-r border-gray-200"></td><td colSpan="6" className="py-1.5 px-4 sm:px-6 text-left"><div className="flex flex-col space-y-1 text-[10px] bg-[#f8fbff] p-2 rounded border border-[#dcebf8]"><div className="flex items-start"><span className="font-bold text-[#1b4379] mr-2 shrink-0">เวลาสอน:</span> <span className="text-gray-700">{session.start_time ? session.start_time.substring(0, 5) : '-'} น. - {session.end_time ? session.end_time.substring(0, 5) : '-'} น.</span></div><div className="flex items-start mt-0.5"><span className="font-bold text-[#1b4379] mr-2 shrink-0">เนื้อหา:</span> <span className="text-gray-600 italic break-words whitespace-pre-wrap leading-tight">{session.topic || '-'}</span></div></div></td></tr>
                      )}
                    </React.Fragment>
                  ))}
                </React.Fragment>
        ...
        <div className="bg-[#fdf3c6] flex justify-between items-center py-2 px-4 border-t border-[#e6d070] shrink-0"><span className="font-bold text-[#8b6508] text-[11px]">รวมรายรับสุทธิ (Total Income)</span><span className="font-bold text-gray-900 text-sm">{totalAmount?.toLocaleString()} บาท</span></div>
      </div>
      ...
      <div className="shrink-0 mt-2">
        ...
        <div className="flex border border-gray-300 bg-gray-50/50 rounded-xs overflow-hidden h-20 sm:h-24">
          ...
          <div className="w-1/2 p-2 flex justify-between items-center">
            <div className="leading-tight flex flex-col justify-center"><p className="text-gray-400 mb-0.5 text-[9px]">ชื่อบัญชี (Account Name)</p><p className="font-bold text-gray-800 text-[10px] sm:text-[11px]">{tutor.account_name || tutor.name || '-'}</p></div>
            <div className="w-16 h-16 sm:w-20 sm:h-20 bg-white flex items-center justify-center p-1 shrink-0 ml-1 border border-gray-200 rounded shadow-sm">
               {/* 🔴 เปลี่ยนกลับมาใช้แท็ก img แบบปกติ */}
               {qrDataUrl ? (
                  <img src={qrDataUrl} crossOrigin="anonymous" alt="Tutor QR Code" className="w-full h-full object-contain" />
               ) : (
